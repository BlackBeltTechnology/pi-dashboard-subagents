...
async function childLoader() {
  if (strategy === "full") { const l = new DefaultResourceLoader({ cwd, agentDir: getAgentDir() }); await l.reload(); return l; }
  if (strategy === "lean") { const l = new DefaultResourceLoader({ cwd, agentDir: getAgentDir(), noSkills: true, noPromptTemplates: true, noThemes: true }); await l.reload(); return l; }
  if (strategy === "wrapper") {

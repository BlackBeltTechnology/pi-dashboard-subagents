...
- **GIVEN** frontmatter has no `model` field, or `model:` is empty, AND `args.model` is undefined
- **WHEN** the subagent is spawned
...
- **GIVEN** the effective ref is `"made-up-model"` (from either source) AND no registry entry matches
...
- **AND** the error message SHALL include the source of the ref (file path or `(tool-call argument)`)

#### Scenario: Tool-call `args.model` takes precedence over frontmatter

- **GIVEN** the resolved `.md` has `model: "@coding"` AND the tool call passes `args.model: "@fast"`
...
- **AND** `createAgentSession` SHALL be called with the model resolved from `@fast`

#### Scenario: Frontmatter `model` applies when `args.model` is absent

- **GIVEN** the resolved `.md` has `model: "@coding"` AND `args.model` is undefined or an empty string
- **WHEN** the extension determines the effective model ref
- **THEN** the resolver SHALL be invoked with `"@coding"`
- **AND** the spawn SHALL behave identically to the pre-change frontmatter-only path
